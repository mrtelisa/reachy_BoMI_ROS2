...
# step -- matches SELECTION_HOLD_SECONDS in reachy_control.py.
...
def _control_phase(cap, landmarker, bomi_map: BoMIMap, cursor_filter: CursorFilter = None,
    ...
    while True:
        ...
        if hand_detected:
            ...
            # the min -- matches reachy_control.py's _teleop_with_grasp_switch.
            lin_vel, ang_vel = compute_dynamic_vel_from_cursor(
                crs_x, crs_y, max_linear=MAX_LINEAR * speed_scale, max_angular=MAX_ANGULAR * speed_scale,
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Dry-run of reachy_control.py's full flow (calibration, "
